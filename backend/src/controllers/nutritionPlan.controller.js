...
const { NutritionPlan, Athlete, User, USER_ROLES } = require('../models');
...
const planSchema = Joi.object({
  ...
  caloriesPerDay: Joi.number().integer().min(800).max(6000).optional(),
  macronutrientRatio: Joi.object({
    carbohydrates: Joi.number().min(0).max(100).optional(),
...
const toDto = (plan) => ({
  ...
  macronutrientRatio: plan.macronutrientRatio,
...
const createPlan = async (req, res, next) => {
  try {
    ...
    if (req.user.role === USER_ROLES.ATHLETE) {
      return res.status(403).json({ message: 'Athletes cannot create nutrition plans' });
    }
    ...
    const fullPlan = await NutritionPlan.findByPk(plan.id, {
      include: [
    ...
    res.status(201).json({ nutritionPlan: toDto(fullPlan) });
  } catch (error) {
    next(error);
  }
};
...
const listPlans = async (req, res, next) => {
  try {
    ...
    res.json({ nutritionPlans: plans.map((plan) => toDto(plan)) });
  } catch (error) {
    next(error);
  }
};
...
const getPlan = async (req, res, next) => {
  try {
    ...
    if (!plan) {
      throw notFound('Nutrition plan not found');
    }
    ...
    res.json({ nutritionPlan: toDto(plan) });
  } catch (error) {
    next(error);
  }
};
...
const updatePlan = async (req, res, next) => {
  try {
    ...
    const reloaded = await NutritionPlan.findByPk(plan.id, {
      include: [
        { model: Athlete, as: 'athlete', include: [{ model: User, as: 'user' }] },
        { model: User, as: 'specialist' }
      ]
    ...
    res.json({ nutritionPlan: toDto(reloaded) });
  } catch (error) {
    next(error);
  }
};
